...
"""
==================================================
   File Name：     kenlm_model.py.py
   email:         songdongdong@weidian.com
   Author :       songdongdong
   date：          2020/7/24 14:39
   Description :  https://github.com/mattzheng/py-kenlm-model
==================================================
"""
...
class kenlm_model():
    def __init__(self, save_path='/Users/songdongdong/workSpace/datas/new_finds/', project='kenlm_base', \
                 memory='50%', min_count=5, order=4, \
                 skip_symbols='"<unk>"', kenlm_model_path='/Users/songdongdong/kenlm/build/bin/'):
        ...
        self.skip_symbols = skip_symbols  # '"<unk>"'
    ...
    def count_ngrams(self):
        ...
        executive_code = self.kenlm_model_path + 'count_ngrams -S %s -o %s --write_vocab_list %s <%s >%s' % (
            self.memory, self.order, self.vocab_file, self.corpus_file, self.ngram_file)
    ...
    def convert_format(self):
        ...
        executive_code = self.kenlm_model_path + 'build_binary -S {} -s {} {}'.format(self.memory, self.arpa_file,
                                                                                      self.klm_file)
        status = os.system(
            executive_code
        )
    ...
    def viterbi(self, nodes):
        ...
        # py3的写法
        paths = nodes[0]
        for l in range(1, len(nodes)):  # 这是个时间步，一步一步的走
            paths_ = paths
            paths = {}
            for i in nodes[l]:  # 这是状态
                nows = {}
                for j in paths_:
    ...
    def cp(self, s):
        ...
        #这里计算的的是，一个整词，减去后面一个字后的分数
        return (self.model.score(' '.join(s), bos=False, eos=False) - self.model.score(' '.join(s[:-1]), bos=False,
                                                                                       eos=False)) or -100.0
    ...
    def mycut(self, s):
        ...
        #     print(s[i], " ", s[i - 1:i + 1], " ", s[i - 2:i + 1], " ", s[i - 3:i + 1])
        #     print(self.cp(s[i]), " ", self.cp(s[i - 1:i + 1]), " ", self.cp(s[i - 2:i + 1]), " ", self.cp(s[i - 3:i + 1]))
    ...
    def read_ngrams(self):
        ...
        #
        ngrams = [Counter({}) for _ in range(self.order)]  # 放入 4 个 ngram  Counter
        total = 0
        size_per_item = self.order * 4 + 8  # 4 * 4 +8（每个ngram前后字符）
        f = open(self.ngram_file, 'rb')
        filedata = f.read()
        filesize = f.tell()  # 返回文件的当前位置，即文件指针当前位置。
        f.close()
        for i in range(0, filesize, size_per_item):
            s = filedata[i: i + size_per_item]
            n = self.unpack('l', s[-8:])  # l: long类型
            if n >= self.min_count:
                total += n
                c = [self.unpack('i', s[j * 4: (j + 1) * 4]) for j in range(self.order)]  # i ： int
    ...
    def filter_ngrams(self, ngrams, total, min_pmi=1):
        ...
        total = float(total)
        for i in range(order - 1, 0, -1):  # [3,2,1]  倒叙
            for w, v in ngrams[i].items():
                # tatal * v  / (  获取当前字的词频  )  * (  获取 )
                # for m in range(i):
                #     print(total*v, "  ==  ",m ,"  === " , w[:m + 1]  ,"  ==  ",ngrams[m].get(w[:m + 1]) , "  ==  ",i-m-1,"  ===  " ,w[m+ 1:],"   == ",ngrams[i - m - 1].get(w[m+ 1:], total))
                #     print()
                # 732.0 == 0 == = 痘 == 8 == 2 === 肌肤 == 366.0
                # 732.0 == 1 == = 痘肌 == 4 == 1 === 肤 == 366.0
                # 732.0 == 2 == = 痘肌肤 == 4 == 0 === == 5

                # 计算公式 pmi    min{total* count_v / ( 痘*肌肤)，total* count_v / ( 痘肌*肤) ,  total* count_v / ( 痘肌肤*)  }
                pmi = min([
    ...
    def compare(self, text_a, text_b):
        ...
        '''
        return self.model.score(' '.join(text_a), bos=False, eos=False) - self.model.score(' '.join(text_b), bos=False,
                                                                                           eos=False)
    ...
    def word_discovery(self, ngrams_dict, \
        ...
        for nw, freq in tqdm(ngrams_dict.items()):
            ...
            if (len(jieba_nw) != 1) and (len([gp for gp in good_pos if gp in ''.join(pos)]) > 0) and (
                    len([bw for bw in bad_words if bw in nw[0]]) == 0):
                new_words_2[nw] = freq
                # print(list(words))
        return new_words_2
...
if __name__ == "__main__":
    ...
    '''
    # 模型加载
    km = kenlm_model(save_path='/Users/songdongdong/workSpace/datas/new_finds/kenlm_base/', project='kenlm_base', \
                     memory='50%', min_count=2, order=4, \
                     skip_symbols='"<unk>"', kenlm_model_path='/Users/songdongdong/kenlm/build/bin/')

    # NLM模型训练文件生成
    text_list = [
        'Whoo 后 拱辰享水 水妍护肤套装整套质地都比较清爽，滋润，侧重保湿，适合各种肤质，调节肌肤水平衡，它还具有修复功效，提亮肤色我是油性肤质用起来也一点也不觉得油腻，味道淡淡的还很好闻，也很好吸收，质地清爽，上脸没有油腻感，就算是炎炎夏日用起来也相当舒服它的颜值很高的，粉粉的超级少女，满足了老夫的少女心保湿指数⭐️⭐️⭐️⭐️⭐️推荐指数⭐️⭐️⭐️⭐️⭐️如果你是油性肤质，选择水妍系列，再也不会担心每天到了下午顶着一个大油饼子脸了，油性、混合性妹子完全不用担心踩雷性价比超高，这一套满足了日常护肤的基本要求(Whoo洁面膏)',
    ...
    # 新词发现
    new_words = km.word_discovery(ngrams_2)
    print(new_words)
    '''
        智能纠错
    '''
    ...
    # # 这里报错：https://deepspeech.bj.bcebos.com/zh_lm/zh_giga.no_cna_cmn.prune01244.klm  自己下载下吧
    # # .pycorrector文件夹是隐藏的，shift+cmd+.可以显示，将下载的2.8G文件拷贝到.pycorrector/datasets文件夹下即可。
    # correct = []
